...
def leaf(zgz, c, d):
    ...
    zgz.chord((15 - chordstartx, 15 + chordstarty, 85 - chordstartx, 85 + chordstarty), d, d + 80, agg.Pen(c, 0), agg.Brush(c))
    zgz.chord((15 + chordendx, 15 - chordendy, 85 + chordendx, 85 - chordendy), d2, d2 + 80, agg.Pen(c, 0), agg.Brush(c))
...
def dot(zgz, a, b):
    zgz.ellipse((a - 5, b - 5, a + 5, b + 5), agg.Brush('gray'))
...
class Blak:
    def __init__(self, x, y, tp, on, num, q):
        ...
        if self.tp == 'sp':
            ...
            c = '#ffeebb'
        self.dib = agg.Dib('RGB', (100, 100), c)
        self.frame = tk.Frame(board, width=100, height=100, bg='', bd=4, highlightthickness=1, highlightbackground='#000')
    ...
    def rendr(self):
        ...
        if self.tp == 'sp':
            ...
            self.dib.ellipse((43, 43, 57, 57), agg.Pen("black", 1), agg.Brush("red"))
        ...
        if self.bid['c'] == 'D' or self.bid['c'] == 'B':
            ...
            if x == rnumb.trn:
                self.frame.bind('<Button-1>', lambda eff: klyk(eff, self.bid['n'], rnumb.a, iface))
        self.frame.bind("<Expose>", lambda e: self.dib.expose(hwnd=e.widget.winfo_id()))
...
# Check for rolling zero
    ...
    if not blocked:
        ...
        if win:
            ...
            game.grid_forget()
            winscreen.grid()
            windib.ellipse((20, 20, 80, 80), agg.Brush(rnumb.trn + rnumb.trn[1:]))
            dotmap(windib, 7)
            x = [22, 36, 50, 64, 78]
            x2 = [31, 43, 57, 69]
            y = [14, 4, 24]
            windib.polygon((29, 30, x[0], y[0], x2[0], y[2], x[1], y[0], x2[1], y[2], x[2], y[1], x2[2], y[2], x[3], y[0], x2[3], y[2], x[4], y[0], 71, 30), agg.Brush('gray'))
            for m in range(5):
                j = 0
                if m == 2:
                    j = 1
                windib.ellipse((x[m] - 2, y[j] - 2, x[m] + 2, y[j] + 2), agg.Brush('gray'))
            reset(jface)
        setb()
        doroll(jface)
        massrend()
...
windib = agg.Dib('RGB', (100, 100), '#6600aa')
winframe = tk.Frame(winscreen, width=100, height=100, bg='', bd=4, highlightthickness=1, highlightbackground='#6600aa')
winframe.bind("<Expose>", lambda e: windib.expose(hwnd=e.widget.winfo_id()))
